...
unsigned long CALLBACK lift_thread(void *p) {
    ...
    while (1) {        /* do forever */
        ...
        printf("Lift: %d Floor %d: Deboarded = %d, ",l.no,l.position,l.stops[l.position]); 
...
unsigned long CALLBACK person_thread(void *p) {
    ...
    while (1) {     /* stay in the building forever */
        ...
        if (to > from) {	/* if we are going up */
            ...
            //---char_at_xy(NLIFTS * 4 + floor[from].waitingtogodown + floor[from].waitingtogoup, NFLOORS - from, 0xdc);
